...
class ProgressTestCase(TestCase):
    ...
    def test_progress(self):
        ...
        self.assertEqual(tabs._progress(self.tab, self.mockuser0, self.course, self.active_page0), [])
        self.assertEqual(tabs._progress(self.tab, self.mockuser1, self.course, self.active_page1)[0].name, 'same')
        self.assertEqual(tabs._progress(self.tab, self.mockuser1, self.course, self.active_page1)[0].link, reverse('progress', args = [self.course.id]))
...
class DiscussionTestCase(TestCase):
	...
	def test_discussion(self):
		...
		if settings.MITX_FEATURES.get('ENABLE_DISCUSSION_SERVICE'):
			...
			self.assertEqual(tabs._discussion(self.tab, self.user, self.course, self.active_page1)[0].name, 'same')
			self.assertEqual(tabs._discussion(self.tab, self.user, self.course, self.active_page1)[0].link, reverse('django_comment_client.forums.views.forum_form_discussion', 
																args = [self.course.id]))
			self.assertEqual(tabs._discussion(self.tab, self.user, self.course, self.active_page1)[0].is_active, True)
			self.assertEqual(tabs._discussion(self.tab, self.user, self.course, self.active_page0)[0].is_active, False)
